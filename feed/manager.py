"""
feed/manager.py — Betclic gRPC price feed (fully async, aiohttp).

Orchestrates one asyncio.Task per match × market.
Pushes OddsUpdate dicts onto the shared asyncio.Queue consumed by server/app.py.

Public interface:
  await run(queue)       — main coroutine, blocks until stop() is called
...
from proto.client import payload_get_match, cmd_list_matches, BASE, HEADERS
...
MAX_MATCH_AGE_H = settings.max_match_age_h
...
async def run(
    ...
    connector = aiohttp.TCPConnector(
        ...
        ssl=False,                 # offering.begmedia.com uses a self-signed cert
    )
